...
#PCA_korrplot_verteilungen_____________________________________________________________________________________________________________________________________

#Korrelation mit Diabetes
data <- na.omit(diabetis_life[,c(unlist(datasort$evaluation5),"NEW_WTH","NEW_WHT","NEW_BMI", "NEW_ABSI")])
data_normalized <- scale(data)
corr_matrix <- cor(data_normalized)

# Save the plot as a PNG file
png(filename = "correlation_plot.png", width = 4000, height = 4000)

# Generate the correlation plot
corrplot(corr_matrix, type = "upper",  tl.col = "black", tl.srt = 45, tl.cex = 2)

# Turn off the device
dev.off()

# Extract the correlations with DERIVAT_DIABETES_DS_HISTORY

# # Extract the correlations with DERIVAT_DIABETES_DS_HISTORY
# cor_with_history <- corr_matrix[, "DERIVAT_DIABETES_DS_HISTORY"]
#
# # Sort the correlations in descending order
# sorted_cor_with_history <- sort(abs(cor_with_history), decreasing = TRUE)
#
# # Display the sorted correlations
# sorted_cor_with_history <- sorted_cor_with_history[-1]
#
# datasort$correlation <- head(sorted_cor_with_history,15)
#Applying PCA
...
fviz_cos2(data.pca, choice = "var", axes = 1:3, title = "", ggtheme = theme_minimal(base_size = 8)) +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust = 0)) +
  ylab("loadings of dim 1 2 & 3")
fviz_cos2(data.pca, choice = "var", axes = 1, title = "", ggtheme = theme_minimal(base_size = 8)) +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust = 0)) +
  ylab("loadings of dim 1")
fviz_cos2(data.pca, choice = "var", axes = 2, title = "", ggtheme = theme_minimal(base_size = 8)) +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust = 0)) +
  ylab("loadings of dim 2")
fviz_cos2(data.pca, choice = "var", axes = 3, title = "", ggtheme = theme_minimal(base_size = 8)) +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust = 0)) +
  ylab("loadings of dim 3")
# fviz_pca_var(data.pca, col.var = "cos2",
#              gradient.cols = c("black", "orange", "green"),
#              repel = TRUE)

# Get the variable contributions and cos2 scores
var_info <- get_pca_var(data.pca)
eig.val <- get_eigenvalue(data.pca)

# Select only the variables with cos2 score > 0.10
selected_vars_dim1 <- var_info$cos2[, 1]
selected_vars_dim1 <- sort(selected_vars_dim1[selected_vars_dim1 > 0.1], decreasing = TRUE)
pcaDim1Names <- names(selected_vars_dim1)

selected_vars_dim2 <- var_info$cos2[, 2]
selected_vars_dim2 <- sort(selected_vars_dim2[selected_vars_dim2 > 0.1], decreasing = TRUE)
pcaDim2Names <-names(selected_vars_dim2)

selected_vars_dim3 <- var_info$cos2[, 3]
selected_vars_dim3 <- sort(selected_vars_dim3[selected_vars_dim3 > 0.01], decreasing = TRUE)
pcaDim3Names <-names(selected_vars_dim3)

datasort$pcaDim1Over01 <- selected_vars_dim1
datasort$pcaDim2Over01 <- selected_vars_dim2
datasort$pcaDim3Over01 <- selected_vars_dim3

save(datasort, file = "datasort.RData")
...
dim(data) - dim(diabetis_life)

filtered_df <- NaFilter(data,0 ,unlist(datasort$evaluation5), filter = FALSE)

filtered_df <-  filtered_df[-which(filtered_df$BS_MISSINGS == 132),]
summary_data <- filtered_df
variables <- c(unlist(datasort$evaluation5), "NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI", "SEX", "age")
length(variables)
100 /dim(summary_data)[1] *
length(which(summary_data$SEX == 1))
100 /dim(summary_data)[1] *
length(which(summary_data$SEX == 2))


ggplot(summary_data, aes(x = age)) +
  geom_histogram(binwidth = 0.5, fill = "steelblue", color = "black") +  # Set bin width to 10
  theme_minimal() +
...
100 /dim(summary_data)[1] *
  length(which(summary_data$age >= 40))
100 /dim(summary_data)[1] *
  length(which(summary_data$DERIVAT_DIABETES_DS_HISTORY == 1))
100 /dim(summary_data)[1] *
  length(which(summary_data$DERIVAT_DIABETES_DS_HISTORY == 0))

#mit 0 sehr häufig
table(summary_data$BS_DEV_WB_FROM_WAIST_S)



dim(filtered_df)
table(filtered_df$BS_MISSINGS)
missing_df <- data.frame(CountOfMissing = filtered_df$BS_MISSINGS)

ggplot(missing_df, aes(x = CountOfMissing)) +
  geom_histogram(binwidth = 0.5, fill = "steelblue", color = "black") +  # Set bin width to 10
  theme_minimal() +
  labs(title = "Histogram von den Fehlwerten",
       x = "Anzahl der Fehlwerte",
       y = "Anzahl der Individuen") +
  scale_x_continuous(breaks = seq(0, 150, by = 10))
# Adjust y&x-axis if needed
#unter alle Fehlwerten
tableHeads <- table(filtered_df$BS_MISSINGS)

tableHeads[which(diff(as.numeric(names(tableHeads))) == max(diff(as.numeric(names(tableHeads)))))]
#111


#unter 100 Fehlwerten
tableHeads <- table(filtered_df$BS_MISSINGS[which(filtered_df$BS_MISSINGS < 100)])

tableHeads[which(diff(as.numeric(names(tableHeads))) == max(diff(as.numeric(names(tableHeads)))))]
#27

#28 um den einen mit 27 zu zeigen
PlotOutlierWithStats(filtered_df, "BS_MISSINGS", 28, FALSE)

#feature & individuen (NA) filter
filtered_df <- NaFilter(summary_data,27,unlist(datasort$evaluation5))
dim(filtered_df)[1] -
  dim(summary_data)[1]

PlotDiffWithStats(summary_data, filtered_df, "NEW_BMI", "", FALSE)

#maha filter______________________________________________________________________________________________________________________________________
not_maha_filtered_df <- MahaFilter(filtered_df, unlist(datasort$evaluation5), 9000,filter = FALSE)

hist(not_maha_filtered_df$mahalanobis_dist)
summary(not_maha_filtered_df$mahalanobis_dist)
#median(not_maha_filtered_df$mahalanobis_dist)+9*IQR(not_maha_filtered_df$mahalanobis_dist)
iqr_fsdfs <- quantile(not_maha_filtered_df$mahalanobis_dist, 0.75) - quantile(not_maha_filtered_df$mahalanobis_dist, 0.25)
median(not_maha_filtered_df$mahalanobis_dist)+99*iqr_fsdfs

ggplot(not_maha_filtered_df, aes(x = mahalanobis_dist)) +
  geom_histogram(binwidth = 0.5, fill = "steelblue", color = "steelblue") +  # Set bin width to 10
  theme_minimal() +
  labs(title = "Histogram der Mahalanobis Distanz",
       x = "Mahalanobis Distanz",
       y = "Frequenz")

quantile(not_maha_filtered_df$mahalanobis_dist, probs = 0.9995)
#4973.349
quantile(not_maha_filtered_df$mahalanobis_dist, probs = 0.95)


cor.test(not_maha_filtered_df$NEW_BMI, not_maha_filtered_df$mahalanobis_dist, method = "pearson")

variables <- c(unlist(datasort$evaluation5), "NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI")
length(variables)
correlations <- list()
for (i in seq_along(variables)) {
  variable_name <- variables[i]
  correlations[i] <- as.numeric(cor.test(not_maha_filtered_df[[variable_name]], not_maha_filtered_df$mahalanobis_dist, method = "pearson")$estimate)
}
o <- order(unlist(correlations))
correlations[o]
variables[o]
...
metrics_table <- data.frame(
  Metrik = variables[o],
  Korrelation = round(unlist(correlations[o]),4)
)
# Print the table
print(metrics_table)
# Optionally, save the table to a CSV file
write.xlsx(metrics_table, file = "calculated_korrelation_maha_table.xlsx", rowNames = FALSE)



ggplot(not_maha_filtered_df, aes(x=not_maha_filtered_df$mahalanobis_dist, y=not_maha_filtered_df$NEW_BMI)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x = "Mahalanobis Distanz", y = "BMI") +
  coord_flip()


# PlotOutlierWithStats(not_maha_filtered_df, "mahalanobis_dist", 4973.349, FALSE)
#
# a_5_maha_filtered_df <- MahaFilter(filtered_df, unlist(datasort$evaluation5), 4973.349)
# maha_top_5SIC <- filtered_df[-which(filtered_df$BS_SIC %in% a_5_maha_filtered_df$BS_SIC),]$BS_SIC
#
# # Add grid lines
# grid(nx = NA, ny = NULL, col = "gray", lty = "dotted")

maha_filtered_df <- not_maha_filtered_df
...
variables <- c(unlist(datasort$evaluation5), "NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI")
length(variables)
#data$NEW_ABSI  <- data$NEW_ABSI * 100
for (i in seq_along(variables)) {
  variable_name <- variables[i]
  data[[variable_name]] <- as.numeric(data[[variable_name]])
  plot <- ggplot(data, aes(x = .data[[variable_name]])) +
    geom_histogram(binwidth = 0.05, fill = "black", color = "black") +  # Set bin width to 10
    theme_minimal() +
    labs(
         title = variable_name,
         x = "",
...
filename <- paste0("all_hist_plot.png")
ggsave(filename, grid_plot,  width = 18, height = 62, dpi = 300, limitsize = FALSE)



mean_filtered_df_5sig <- MeanSdFilter(maha_filtered_df,unlist(datasort$evaluation5),5,TRUE )

mean_filtered_df <- mean_filtered_df_5sig

# mean_filtered_df_4sig <- MeanSdFilter(maha_filtered_df,unlist(datasort$evaluation5),4,TRUE )
# mean_filtered_df_3sig <- MeanSdFilter(maha_filtered_df,unlist(datasort$evaluation5),3,TRUE )
#
# 100/(9300 * (132))*sum(mean_filtered_df_5sig$max_outlier + mean_filtered_df_5sig$min_outlier)
# 100/(9300 * (132))*sum(mean_filtered_df_4sig$max_outlier + mean_filtered_df_4sig$min_outlier)
# 100/(9300 * (132))*sum(mean_filtered_df_3sig$max_outlier + mean_filtered_df_3sig$min_outlier)

sum(mean_filtered_df_5sig$max_outlier + mean_filtered_df_5sig$min_outlier)

mean_filtered_df$sum_outliers <- mean_filtered_df_5sig$max_outlier + mean_filtered_df_5sig$min_outlier

# Flattening the list of vectors into a single vector
flattened_outliers <- unlist(mean_filtered_df[order(mean_filtered_df$sum_outliers, decreasing = TRUE),]$max_outlier_detail)
# Creating a frequency table
outlier_counts <- table(flattened_outliers)
# Sorting the table in decreasing order
sorted_outlier_counts <- sort(outlier_counts, decreasing = TRUE)
# Print the sorted table
print(sorted_outlier_counts)

plots <- list()
for (i in c(1:length(unlist(c(datasort$evaluation5))))) {
  plots[[i]] <- PlotBoxViolinScatterMean(maha_filtered_df, unlist(c(datasort$evaluation5))[[i]],5,FALSE)
}

#PlotBoxViolinScatterMean(maha_filtered_df, unlist(c("BS_HEAD_HT"))[[1]],5,FALSE)
# Combine plots into a grid
grid_plot <- do.call(grid.arrange, c(plots, ncol = 6))

filename <- paste0("allbox-violin_plots_scatter_5meansd.png")

ggsave(filename, grid_plot,  width = 18, height = 62, dpi = 300, limitsize = FALSE)


plots <- NULL
#create_violin_plot_diabetes_not_diabetes
for (i in c(1:length(unlist(c(datasort$evaluation5,"NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI"))))) {
  plots[[i]] <- SymetricViolinBoxPlot(mean_filtered_df[which(mean_filtered_df$DERIVAT_DIABETES_DS_HISTORY == 1),unlist(c(datasort$evaluation5,"NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI"))[[i]]],mean_filtered_df[which(mean_filtered_df$DERIVAT_DIABETES_DS_HISTORY == 0),unlist(c(datasort$evaluation5,"NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI"))[[i]]], "","",unlist(c(datasort$evaluation5,"NEW_BMI", "NEW_WHT", "NEW_WTH", "NEW_ABSI"))[i])
}
# Combine plots into a grid
grid_plot <- do.call(grid.arrange, c(plots, ncol = 6))
filename <- paste0("allbox-violin_diabetes_plotsgrid_plot.png")
ggsave(filename, grid_plot,  width = 18, height = 62, dpi = 300, limitsize = FALSE)
